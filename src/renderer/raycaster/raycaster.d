...
class Raycaster : Engine
{
		...
		foreach(r, ref row; taskPool.parallel(this.backBuffer.raw))
		{
			...
			//	Quaternion cameraRotationAngle = Quaternion(this.activeCamera.rotation/2f, this.activeCamera.direction).unit;
				...
				if(r == 350 && c == 500 && false) {
					...
					writeln("CamDirection: ", this.activeCamera.direction);
				//	writeln("CamRotation: ", cameraRotation);
				//	writeln("Rotation: ", rayRotation * cameraRotation);
	...
	private Tuple!(Vector, Vector) intersections(Vector ray, Vector delta, Vector size)
	{
		...
		Vector intersectionNear = (nearPlanes - this.activeCamera.position) / ray.unit;
		Vector intersectionFar = (farPlanes - this.activeCamera.position) / ray.unit;
	...
	private VOctreeNode[] traverseTree(Vector ray, ulong row, ulong column, int depth = 0)
	{
		...
		if(row == 350 && column == 500 && false)
		{
			writeln("Cam pos: ", this.activeCamera.position);
			writeln("Cam dir: ", this.activeCamera.direction);
			writeln("Cam focus: ", this.activeCamera.focus);
			writeln("Ray: ", ray);
			writeln("Intersection Near: ", intersections[0]);
			writeln("Intersection Far: ", intersections[1]);
			writeln("dNear: ", dNear);
			writeln("dFar: ", dFar);
		}
